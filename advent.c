...
void scanint(int *pi, char *str) {
  while (*str) {
    if ((sscanf(str, "%d,", pi++)) < 1) bug(41); /* failed before EOS	*/
...
void saveadv(void) {
  ...
  fflush(stdout);
  if (NULL == gets(username)) exit(0);
  if (sptr = strchr(username, '.')) *sptr = '\0'; /* kill extension	*/
  if (strlen(username) > 8) username[8] = '\0';   /* max 8 char filename	*/
  strcat(username, ".adv");
#else
  (void)sptr;
  game_name(username);
...
char *game_name(char *filename) {
  ...
  printf("What is the name of the saved game? ");
  fflush(stdout);
  if (NULL == fgets(username, 32, stdin)) exit(0);
  if ((sptr = strchr(username, '\n'))) *sptr = '\0';
  if ((sptr = strchr(username, '\r'))) *sptr = '\0';
  if ((sptr = strchr(username, '.'))) *sptr = '\0'; /* kill extension	*/
  if (strlen(username) > 8) username[8] = '\0';     /* max 8 char filename	*/
  strcat(username, ".adv");
