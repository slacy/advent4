...
void gettrav(int loc) {
  ...
  while ((aptr = rindex(atrav, ','))) *aptr = '\0'; /* terminate substring
                                                       */
  aptr = &atrav[0];
  for (i = 0; i < MAXTRAV; ++i) {
    ...
    if (!(*aptr)) {
      ...
      if (dbugflg)
        for (i = 0; i < MAXTRAV; ++i)
          printf("cave[%d] = %d %d %d\n", loc, travel[i].tdest, travel[i].tverb,
                 travel[i].tcond);
...
int yes(int msg1, int msg2, int msg3) {
  ...
  fputc('>', stdout);
  fflush(stdout);
  if (NULL == fgets(answer, 80, stdin)) exit(0);
...
void rspeak(int msg) {
  ...
  if (msg == 54)
    ...
    if (dbugflg) printf("Seek loc msg #%d @ %ld\n", msg, idx4[msg]);
...
void pspeak(int item, int state) {
  ...
  if (p == NULL)
    ...
    while (n--)
      while ((c = *p++) != '/')
        if (c == '\0') bug(32);
    for (n = 0; p[n] != '\0' && p[n] != '/'; n++) putchar(p[n]);
  }
#else
...
*/
int here(int item) { return (place[item] == loc || toting(item)); }

/*
        Routine to tell if an item is being carried.
*/
int toting(int item) { return (place[item] == -1); }

/*
        Routine to tell if a location causes
        a forced move.
*/
int forced(int atloc) { return (cond[atloc] == 2); }
...
*/
int at(int item) { return (place[item] == loc || fixed[item] == loc); }
...
*/
int liqloc(int loc) {
  if (cond[loc] & LIQUID) return (liq2(cond[loc] & WATOIL));
