...
void descitem(void) {
  ...
  if (tally == tally2 && tally != 0 && limit > 35) limit = 35;
}
  ...
  for (kk = 0; travel[kk].tdest >= 0 && !mvflag; ++kk) {
    ...
    switch (rcond / 100) {
      case 0:
        ...
        if (rcond && dbugflg) printf("%% move %d %d\n", pctt, mvflag);
        break;
      case 1:
        if (robject == 0)
          ++mvflag;
      ...
      case 5:
      case 7:
        if (prop[robject] != (rcond / 100) - 3) ++mvflag;
        break;
      default:
        bug(37);
    }
...
void doobj(void) {
  ...
  */
  if (fixed[object] == loc || here(object)) trobj();
  /*
...
void dwarves(void) {
  ...
  */
  if (newloc == 0 || forced(newloc) || cond[newloc] & NOPIRAT) return;
  /*
  ...
  */
  if (dflag == 1) {
    if (newloc < 15 || pct(95)) return;
    ++dflag;
    for (i = 1; i < 3; ++i)
      if (pct(50)) dloc[rand() % 5 + 1] = 0;
    for (i = 1; i < (DWARFMAX - 1); ++i) {
      if (dloc[i] == newloc) dloc[i] = daltloc;
      odloc[i] = dloc[i];
    }
    rspeak(3);
    drop(AXE, newloc);
  ...
  for (i = 1; i < DWARFMAX; ++i) {
    ...
    */
    ...
    if (i == 6)
      ...
      if (odloc[i] == dloc[i]) {
        ++attack;
        if (knfloc >= 0) knfloc = newloc;
        if (rand() % 1000 < 95 * (dflag - 2)) ++stick;
      }
    }
  }
  if (dtotal == 0) return;
...
void dopirate(void) {
  int j, k;
  if (newloc == chloc || prop[CHEST] >= 0) return;
  k = 0;
  for (j = 50; j <= MAXTRS; ++j)
    if (j != PYRAMID ||
  ...
  if (tally == tally2 + 1 && k == 0 && place[CHEST] == 0 && here(LAMP) &&
      prop[LAMP] == 1) {
    rspeak(186);
    move(CHEST, chloc);
    move(MESSAGE, chloc2);
...
stealit:
  ...
  rspeak(128);
  if (place[MESSAGE] == 0) move(CHEST, chloc);
  move(MESSAGE, chloc2);
  for (j = 50; j <= MAXTRS; ++j) {
    if (j == PYRAMID && (newloc == place[PYRAMID] || newloc == place[EMERALD]))
      continue;
    if (at(j) && fixed[j] == 0) carry(j, newloc);
...
int stimer(void) {
  ...
  if (clock1 == 0) {
    ...
    */
    prop[GRATE] = 0;
    prop[FISSURE] = 0;
    for (i = 1; i < DWARFMAX; ++i) dseen[i] = 0;
    move(TROLL, 0);
    move((TROLL + MAXOBJ), 0);
    move(TROLL2, 117);
  ...
  if (limit <= 30) {
    if (lmwarn || !here(LAMP)) return (0);
    lmwarn = 1;
    i = 187;
    if (place[BATTERIES] == 0) i = 183;
    if (prop[BATTERIES] == 1) i = 189;
    rspeak(i);
    return (0);
  }
  return (0);
