...
void vtake(void) {
  ...
  */
  msg = 25;
  if (object == PLANT && prop[PLANT] <= 0) msg = 115;
  if (object == BEAR && prop[BEAR] == 1) msg = 169;
  if (object == CHAIN && prop[BEAR] != 0) msg = 170;
  if (fixed[object]) {
    rspeak(msg);
    return;
  }
  ...
  */
  i = liq();
  if (object == BOTTLE && i != 0) place[i] = -1;
  rspeak(54);
}
...
void vdrop(void) {
  ...
  */
  ...
  if (object == BOTTLE && i != 0) place[i] = 0;
  /*
     handle bird and cage
  */
  if (object == CAGE && prop[BIRD] != 0) drop(BIRD, loc);
  if (object == BIRD) prop[BIRD] = 0;
  drop(object, loc);
}
