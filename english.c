...
int english(void) {
  ...
  if (*word2)
    if (!analyze(word2, &type2, &val2)) return (0); /* didn't know it	*/
...
void getwords(void) {
  ...
  word1[0] = word2[0] = '\0';
  fflush(stdout);
  if (NULL == fgets(words, 80, stdin)) exit(0);
  wptr = words;
  while ((*wptr = tolower(*wptr))) ++wptr;
